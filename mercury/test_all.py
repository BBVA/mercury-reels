...
def test_intake():
    data, targets = create_dataframes()

    evn = reels.Events()
    assert evn.num_events() == 0
    itk = reels.Intake(data)
    itk.insert_rows(evn, ['Emitter', 'Description', 'Weight'])
    assert evn.num_events() == 7

    st = itk.__str__()
    assert st == itk.__repr__()
    assert st.startswith('reels.Intake')

    cli = reels.Clients()
    clp = reels.Clips(cli, evn, time_format = '%Y-%m-%d %H:%M:%S')

    assert clp.num_events() == 0

    itk = reels.Intake(data)
    itk.scan_events(clp, columns = ['Emitter', 'Description', 'Weight', 'Client', 'Time'])

    assert clp.num_events() > 4000
    assert clp.num_clips() == 999

    trg = reels.Targets(clp)
    assert trg.num_targets() == 0
    itk = reels.Intake(targets)
    itk.insert_targets(trg, columns = ['Client', 'Time'])
    assert trg.num_targets() > 200

    df = pd.DataFrame(list(zip(['e', 'f', 'g'], ['dx', 'dy', 'dz'], [0.11, 0.22, 0.33], [1, 2, 3])),
                               columns = ['emitter', 'description', 'weight', 'code'])

    evn = reels.Events()
    assert evn.num_events() == 0
    itk = reels.Intake(df)
    itk.define_events(evn)
    assert evn.num_events() == 3
    ...
    evn = reels.Events()
    assert evn.num_events() == 0
    itk = reels.Intake(spark_data, spark_method = 'local_iterator')
    itk.insert_rows(evn, ['Emitter', 'Description', 'Weight'])
    assert evn.num_events() == 7
    assert not itk.use_accumulator
    ...
    evn = reels.Events()
    assert evn.num_events() == 0
    itk = reels.Intake(spark_data, spark_method = 'accumulator')
    itk.insert_rows(evn, ['Emitter', 'Description', 'Weight'])
    assert evn.num_events() == 7
    assert itk.use_accumulator
    ...
    spark_targets = SPARK.createDataFrame(targets)

    trg = reels.Targets(clp)
    assert trg.num_targets() == 0
    itk = reels.Intake(spark_targets)
    itk.insert_targets(trg, columns = ['Client', 'Time'])
    assert trg.num_targets() > 200
    assert not itk.use_accumulator
